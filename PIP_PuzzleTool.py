...
class PIP_DynamicPuzzleSelection:
    def __init__(self):
        pass
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "image1": ("IMAGE",),
                "string1": ("STRING", {"default": ""}),
                "selection_string": ("STRING", {"default": ""}),
                "inter_image_margin": ("INT", {
                    "default": 50,
                    "min": 0,
                    "max": 200,
                ...
                "use_english_description": ("BOOLEAN", {"default": True}),
            },
            "optional": {
                "image2": ("IMAGE",),
                "string2": ("STRING", {"default": ""}),
    ...
    def select_and_stitch(self, image1, string1, selection_string, inter_image_margin, image_height, 
        ...
        selection_items = [item.strip() for item in selection_string.split("|") if item.strip()]
        ...
        for idx, (img, text) in enumerate([
            ...
            if img is not None:
                ...
                # Only take the first image from each batch
                img_tensor = img[0]
                original_images_data.append((img_tensor, text))
        
        # Filter images based on selection string
        if selection_items:
            # Only keep images whose titles are in the selection list
            filtered_images_data = [(img, text) for img, text in original_images_data if text.strip() in selection_items]
            
            # Sort according to the order in selection_items
            images_data = []
            for selection in selection_items:
                for img, text in filtered_images_data:
                    if text.strip() == selection:
                        images_data.append((img, text))
                        break
        else:
            # If no selection string provided, use all images
            images_data = original_images_data
        
        # If no images left after filtering, return empty image and message
        if not images_data:
    ...
    def _generate_english_description(self, titles):
        ...
        # Format: From left to right, this picture shows: title1|title2|title3|...
        title_part = "|".join(valid_titles)
        description = f"From left to right, this picture shows: {title_part}"
